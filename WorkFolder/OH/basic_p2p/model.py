...
def model_p2p(data):
    ...
    model.P = Set(within=model.H*model.H) # Subset of network in the community (dim=2, H, H) (eg. P={(1,2), (1,3), (2,1), (2,3), (3,1), (3,2)})
...
def generate_data_dict(file_path_data, start_date_str, end_date_str, n_houses, houses_pv, houses_bat):
...
data = generate_data_dict(file_path_data, start_date_str, end_date_str, n_houses, houses_pv, houses_bat)
...
x_g_dict = instance.x_g.get_values()
# Then you can convert it to dataframe
x_g_df = pd.DataFrame.from_dict(x_g_dict, orient="index")
x_g_df.columns = ["x_g"] # set a name for the dataframe
...
x_g_df = x_g_df.reset_index()
# Then, create one column for each element
x_g_df[['Time', 'Household']] = pd.DataFrame(x_g_df['index'].tolist(), index=x_g_df.index)
# Eliminate the index column containing the tuple
x_g_df = x_g_df.drop(columns='index')
# Set the columns as index again
x_g_df = x_g_df.set_index(["Time", "Household"])

# Then you can plot the results or save it as excel
x_g_df.to_csv(file_path_results + "x_g.csv")

fig, ax = plt.subplots(figsize=(12,7))

x = x_g_df.index.get_level_values(0).unique() # Get unique values for time, this will be the x-axis

for house in x_g_df.index.get_level_values(1).unique():
    y = x_g_df[x_g_df.index.get_level_values(1) == house].values
